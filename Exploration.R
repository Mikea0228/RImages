...
for (i in 2:(MyDim[2] - 1)) {
  for (j in 2:(MyDim[1] - 1)) {
    MyImage.recon[i,j,] <- 0.5 * (((MyDim[2]-j) * MyImage.border[i,1,] + j* MyImage.border[i,MyDim[2],]) /MyDim[2]
                                  +
                                    ((MyDim[1]-i) * MyImage.border[1,j,] + i* MyImage.border[MyDim[2],j,])/MyDim[1]  
                                  )
